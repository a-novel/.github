...
> while lower layers should receive them statically – in particular, don't generate values in postgres or
...
Handler tests mock the service dependency and assert protocol-level behavior (status codes, response bodies).

##### HTTP handlers

Use `net/http/httptest` to build requests and record responses. POST handlers receive a JSON body via
`strings.NewReader`, GET handlers pass query parameters in the URL:

```go
// POST request.
httptest.NewRequest(http.MethodPost, "/", strings.NewReader(`{"email":"user@provider.com"}`))

// GET request with query parameters.
httptest.NewRequest(http.MethodGet, "/?id=00000000-0000-0000-0000-000000000001", nil)

// GET request with repeated parameters (slices).
httptest.NewRequest(http.MethodGet, "/?limit=10&roles=role1&roles=role2", nil)
```

The test loop calls `handler.ServeHTTP(w, testCase.request)` and asserts the recorded status code. When the
handler returns a JSON body, read it and compare against untyped structures (`map[string]any`, `[]any`):
...
for _, testCase := range testCases {
	t.Run(testCase.name, func(t *testing.T) {
		...
		service := handlersmocks.NewMockEntityActionService(t)
		...
		handler := handlers.NewEntityAction(service, config.LoggerDev)
...
Test cases should declare `expectStatus int` and `expectResponse any`. Each handler error mapping should be
covered (domain error -> expected HTTP status), plus a generic `Error/Internal` case using `errFoo`.

##### gRPC handlers

gRPC handlers are called directly as Go methods — no HTTP layer is involved. The test passes a protobuf request
and asserts the gRPC status code extracted from the returned error:
...
for _, testCase := range testCases {
	t.Run(testCase.name, func(t *testing.T) {
		...
		handler := handlers.NewEntityAction(service)

		res, err := handler.EntityAction(t.Context(), testCase.request)
		resSt, ok := status.FromError(err)
		require.True(t, ok)
		require.Equal(t, testCase.expectStatus, resSt.Code())
...
Test cases should declare `expectStatus codes.Code` and `expect *protogen.EntityActionResponse`. Response
comparison is done directly on protobuf structs (no JSON unmarshalling needed). Use `status.FromError(err)`
from `google.golang.org/grpc/status` to extract the gRPC code.
